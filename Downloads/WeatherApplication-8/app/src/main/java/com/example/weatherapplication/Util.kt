...
object Util {
    ...
    fun setShowNotification(context: Context, show: Boolean) {
        context.getSharedPreferences("WeatherApplication", Context.MODE_PRIVATE)
            .edit().apply { putBoolean("notification", show); apply() }
    }
    fun getShowNotification(context: Context): Boolean {
        context.getSharedPreferences("WeatherApplication", Context.MODE_PRIVATE)
            ?.getBoolean("notification", false)?.let { return it }
        return false
    }
}